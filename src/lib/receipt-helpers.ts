...
export async function postReceipt(
  ...
  await client.query(
    `UPDATE inventory_receipts
     ...
     WHERE id = $1`,
    [receiptId, options.postedBy ?? null, Boolean(options.autoAllocate)]
  )
