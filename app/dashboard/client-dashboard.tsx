...
export default function DashboardClient({ data }: DashboardClientProps) {
  ...
  const receiptsDraft = data.inventoryReceipts.filter((item) => item.status === 'DRAFT').length;
  ...
  data.inventoryReceipts.forEach((receipt) => {
    const key = receipt.postedBy || 'Sem registro';
  ...
  const separatedSet = useMemo(
    () =>
      new Set(
        orders
          .filter((order) => order.items.every((item) => (item.qtySeparated ?? 0) >= (item.qtyRequested ?? 0)))
          .map((order) => order.id)
      ),
    [orders]
  );

  const producingSet = useMemo(() => new Set(productionTasks.map((task) => task.orderId)), [productionTasks]);
  ...
  const finishedSet = useMemo(() => {
    ...
    orders.forEach((order) => {
      const allSeparated = order.items.every((item) => (item.qtySeparated ?? 0) >= (item.qtyRequested ?? 0));
      const tasksForOrder = productionTasks.filter((task) => task.orderId === order.id);
      const allProdDone = tasksForOrder.length > 0 && tasksForOrder.every((task) => task.status === 'DONE');
      if (allSeparated || allProdDone) set.add(order.id);
    });
    return set;
  }, [orders, productionTasks]);
  ...
  const ordersComparisonSeries = useMemo(() => {
    ...
    return buckets.map((bucket) => {
      ...
      const inSeparation = ordersOnBucket.filter((order) => {
        const totalRequested = order.items.reduce((sum, item) => sum + (item.qtyRequested ?? 0), 0);
        const totalSeparated = order.items.reduce((sum, item) => sum + (item.qtySeparated ?? 0), 0);
        return totalSeparated > 0 && !finishedSet.has(order.id);
      }).length;
      const finalized = ordersOnBucket.filter((order) => finishedSet.has(order.id)).length;
      return { date: bucket, created, inSeparation, finalized };
    });
  }, [orders, period, selectedMonth, finishedSet]);

  const partialSeparatedSet = useMemo(() => {
    const set = new Set<string>();
    orders.forEach((order) => {
      const totalRequested = order.items.reduce((sum, item) => sum + (item.qtyRequested ?? 0), 0);
      const totalSeparated = order.items.reduce((sum, item) => sum + (item.qtySeparated ?? 0), 0);
      if (totalSeparated > 0 && totalSeparated < totalRequested) {
        set.add(order.id);
      }
    });
    return set;
  ...
  const partialSeparated = Array.from(partialSeparatedSet).filter(
    (id) => !producingSet.has(id) && !finishedSet.has(id)
  ).length;
  ...
  const waitingProduction = separatedOnly;
  const stoppedInProduction = blockedCount;
  const ordersInSeparation = separatedOnly + partialSeparated;
  const stoppedInSeparation = orders.filter((order) => {
    const totalSeparated = order.items.reduce((sum, item) => sum + (item.qtySeparated ?? 0), 0);
    const hasProduction = producingSet.has(order.id);
    const isFinished = finishedSet.has(order.id);
      ...
      <div className="space-y-6">
        <section aria-labelledby="overview">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            ...
            />
            <KpiCard
              title="Recebimentos (rascunho)"
              value={receiptsDraft}
              icon={Package}
              tone="success"
              onClick={() => router.push('/inventory?view=draft')}
            />
            <KpiCard
